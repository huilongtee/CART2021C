...
@section('content')
    ...
    <script>
        function cal() {
            ...
            var len = cboxes.length; //get nunmber of cid[] checkbox inside the page
            for (var i = 0; i < len; i++) {
                if (cboxes[i].checked) { // calcuklate if checked
                    subtotal = parseFloat(names[i].value) + parseFloat(subtotal);
                }
            }
            document.getElementById('sub').value = subtotal.toFixed(2); //2 decimal point
        }
    </script>
                    ...
                    <tbody>
                        @foreach ($products as $product)
                            <tr>
                                ...
                                <td>
                                    <input type="checkbox" name="cid[]" value="{{ $product->cid }}" onclick="cal()">
